...
class ImageInsightsOpenSearchStack(Stack):

  def __init__(self, scope: Construct, construct_id: str, vpc, sg_bastion_host, **kwargs) -> None:
    super().__init__(scope, construct_id, **kwargs)

    sg_use_opensearch = aws_ec2.SecurityGroup(self, "ImageTagSearchClientSG",
      vpc=vpc,
      allow_all_outbound=True,
      description='security group for elasticsearch client of image tagger',
      security_group_name='use-image-tagger-es'
    )
    cdk.Tags.of(sg_use_opensearch).add('Name', 'sg-use-image-tagger-es')

    sg_opensearch_cluster = aws_ec2.SecurityGroup(self, "ImageTagSearchSG",
      vpc=vpc,
      allow_all_outbound=True,
      description='security group for elasticsearch of image tag',
      security_group_name='image-tagger-es'
    )
    cdk.Tags.of(sg_opensearch_cluster).add('Name', 'sg-image-tagger-es')
    ...
    ops_domain_name = "image-insights"
    ...
    ops_domain = aws_opensearchservice.Domain(self, "ImageTagSearch",
    ...
    cdk.CfnOutput(self, 'OPSDomainEndpoint', value=self.search_domain_endpoint, export_name='SearchDomainEndpoint')
    cdk.CfnOutput(self, 'OPSDashboardsURL', value=f"{self.search_domain_endpoint}/_dashboards/", export_name='SearchDashboardsURL')
